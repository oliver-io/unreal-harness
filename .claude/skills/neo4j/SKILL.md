...
**A runnable, generic ingester ships with the harness: `tools/neo4j/`** (wrapped by
`scripts/neo4j.{ps1,sh}`). It implements everything below — discover from the live
...
```bash
scripts/neo4j.ps1 discover                       # what exists + reader coverage
scripts/neo4j.ps1 ingest --class StateTree       # graph all StateTrees (additive)
scripts/neo4j.ps1 ingest --dir /Game/Foo/ --with-refs   # a subtree + cross-asset edges
...
- The harness **ships a working ingester** at `tools/neo4j/` (Bun/TS, wrapped by
...
- **Typed ontological shapers** (`tools/neo4j/src/ontology.ts`) produce the curated schema
  ...
  generic walk (`tools/neo4j/src/shape.ts`); other classes list as *(no reader)*.

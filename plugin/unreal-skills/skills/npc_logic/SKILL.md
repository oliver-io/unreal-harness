...
- **Not a deep-domain replacement.** For replication depth use `networking`; for standing tests use `automated-tester`; for a written architecture document use `architect`; for EQS/StateTree tool mechanics see `docs/USAGE.md`. Cross-reference, don't re-derive.

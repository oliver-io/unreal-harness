 ...
 *
 * This is the project-agnostic exemplar for a "verify-<game>.ts" script. Copy it,
 * swap the REPLACE_* placeholders for your game's actors / log markers, and drop it
 * at `src/server/scripts/verify-<game>.ts` in your project. It connects to the running
 * MCP server as a REAL MCP client, drives a self-playing scenario in PIE, asserts on
 * log markers + pie_query positions, screenshots, stops PIE, prints PASS/FAIL, and
 ...
 * See docs/TESTING.md for the doctrine, or run the /automated-tester skill to scaffold.
 */
...
try {
  ...
  //    Replace "REPLACE_actor" with a filter that matches your moving actor.

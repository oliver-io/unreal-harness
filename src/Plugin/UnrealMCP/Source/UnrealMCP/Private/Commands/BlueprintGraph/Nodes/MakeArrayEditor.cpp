...
bool FMakeArrayEditor::AddArrayElementPin(UK2Node* Node, UEdGraph* Graph)
{
	if (!Node || !Graph)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Invalid node or graph in AddArrayElementPin"));
		return false;
	}
	...
	if (!MakeArrayNode)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Node is not a UK2Node_MakeArray"));
		return false;
	}
...
bool FMakeArrayEditor::RemoveArrayElementPin(UK2Node* Node, UEdGraph* Graph, const FString& PinName)
{
	if (!Node || !Graph)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Invalid node or graph in RemoveArrayElementPin"));
		return false;
	}
	...
	if (!PinToRemove)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Pin not found: %s"), *PinName);
		return false;
	}

	// RemoveInputPin asserts the pin is a top-level input pin (check on Direction and
	// ParentPin). Reject the output pin or a split sub-pin here instead of crashing.
	if (PinToRemove->Direction != EGPD_Input || PinToRemove->ParentPin != nullptr)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Pin is not a removable array element: %s"), *PinName);
		return false;
	}
	...
	if (InputPinCount <= 1)
	{
		UE_LOG(LogUnrealMCP, Warning, TEXT("Cannot remove the last array element pin"));
		return false;
	}
...
bool FMakeArrayEditor::SetNumArrayElements(UK2Node* Node, UEdGraph* Graph, int32 NumElements)
{
	if (!Node || !Graph)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("Invalid node or graph in SetNumArrayElements"));
		return false;
	}
	...
	if (NumElements < 1)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("MakeArray must have at least 1 element pin"));
		return false;
	}
	...
	while (CurrentElementCount > NumElements)
	{
		// Find the last input pin and remove it. RemoveInputPin asserts the pin is a
		// top-level input pin (ParentPin == nullptr), so never select a split sub-pin.
		UEdGraphPin* LastInputPin = nullptr;
		for (UEdGraphPin* Pin : MakeArrayNode->Pins)
		{
			if (Pin && Pin->Direction == EGPD_Input && Pin->ParentPin == nullptr && Pin->PinName != UEdGraphSchema_K2::PC_Wildcard)
			{
				LastInputPin = Pin;
			}
		}

...
UK2Node* FDataNodeCreator::CreateVariableGetNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("CreateVariableGetNode: variable_name length %d exceeds FName max %d"), VariableName.Len(), NAME_SIZE - 1);
		return nullptr;
	}
...
UK2Node* FDataNodeCreator::CreateVariableSetNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	...
	if (VariableName.Len() >= NAME_SIZE)
	{
		UE_LOG(LogUnrealMCP, Error, TEXT("CreateVariableSetNode: variable_name length %d exceeds FName max %d"), VariableName.Len(), NAME_SIZE - 1);
		return nullptr;
	}

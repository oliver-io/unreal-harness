...
class UNREALMCP_API FBlueprintNodeManager
{
	...
	private:
		...
		/**
		 * Create an AnimGraph node (SequencePlayer, BlendSpacePlayer, AimOffset, Slot, etc.)
		 * Sets position and animation asset if provided.
		 */
		template<typename T>
		static T* CreateAnimGraphNode(class UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params);
